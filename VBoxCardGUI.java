...
public class VBoxCardGUI extends VBox {
    ...
    public VBoxCardGUI(BoardSquare bSquare) {
        ...
        switch (boardSquareCardData[2]) {
            case "RED":
                cardColor = Color.FIREBRICK;
                break;
            case "BLUE":
                cardColor = Color.DARKCYAN;
                break;
            case "GREEN":
                cardColor = Color.DARKGREEN;
                break;
            default:
        ...
        for (int i = 1; i <= shapeNumber; i++) {
            switch (boardSquareCardData[0]) {
                case "OVALS":
                    shape = createEllipse(cardColor, cardFill);
                    break;
                case "DIAMONDS":
                    shape = createDiamond(cardColor, cardFill);
                    break;
                case "SQUIGGLES":
                    shape = createCurve(cardColor, cardFill);
                    break;
                default:
                    shape = createEllipse(cardColor, cardFill);
                    break;
